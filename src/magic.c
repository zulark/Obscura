...
void MagicUpdate(Player *player, Particle *particles, float *cooldowns)
{
    for (int i = 0; i < MAGIC_COUNT; i++)
    {
        if (magics[i].currentCooldown > 0.0f)
            magics[i].currentCooldown -= GetFrameTime();
        if (magics[i].currentCooldown < 0.0f)
            magics[i].currentCooldown = 0.0f;
        if (cooldowns)
            cooldowns[i] = magics[i].currentCooldown;
    }
...
int MagicCast(Player *player, int slot, Particle *particles, Vector2 targetPos, Enemy *enemies, int maxEnemies, Boss *boss)
{
    ...
    if (magics[slot].id == MAGIC_AREA_ATTACK)
    {
        ...
        if (boss && boss->active && boss->health > 0)
        {
            ...
            float dist = Vector2Distance(bossCenter, targetPos);
            if (dist <= areaRadius)
            {
                BossTakeDamage(boss, 350);
            }
    ...
    else if (magics[slot].id == MAGIC_BARRIER)
    {
        ...
        AudioPlaySound(SOUND_PLAYER_MAGIC_DASH);
    }
...
{
    if (magicId < 0 || magicId >= MAGIC_COUNT)
        return 0;
